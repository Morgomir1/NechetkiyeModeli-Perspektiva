...
def plot_task1():
    ...
    ax.stem(x_vals, mu_vals, basefmt=' ', linefmt='k-', markerfmt='ko')
...
def plot_task2():
    ...
    ax1.plot(x_a, mu_a, 'k-', linewidth=2)
    ...
    ax2.plot(x_b, mu_b, 'k-', linewidth=2)
...
def plot_task3():
    ...
    ax.plot(x_approx2, mu_approx2, 'k-', linewidth=2, marker='o', label='Примерно 2', linestyle='-')
    ax.plot(x_approx3, mu_approx3, 'k--', linewidth=2, marker='s', label='Примерно 3', linestyle='--')
    ax.plot(x_sum, mu_sum, 'k-.', linewidth=2, marker='^', label='Сумма', linestyle='-.')
...
def plot_task4():
    ...
    ax.plot(x_fuzzy2, mu_fuzzy2, 'k-', linewidth=2, marker='o', label='Нечеткая 2', linestyle='-')
    ax.plot(x_fuzzy_minus2, mu_fuzzy_minus2, 'k--', linewidth=2, marker='s', label='Нечеткая -2', linestyle='--')
    ax.plot(x_sum, mu_sum, 'k-.', linewidth=2, marker='^', label='Сумма', linestyle='-.')
...
def plot_task7():
    ...
    for i, alpha in enumerate(alpha_values):
        ax1.plot([add_left[i], add_right[i]], [alpha, alpha], 'k-', linewidth=2)
        ax1.plot(add_left[i], alpha, 'ko', markersize=6)
        ax1.plot(add_right[i], alpha, 'ko', markersize=6)
    ax1.set_xlabel('x', fontsize=12)
    ...
    # Вычитание
    for i, alpha in enumerate(alpha_values):
        ax2.plot([sub_left[i], sub_right[i]], [alpha, alpha], 'k-', linewidth=2)
        ax2.plot(sub_left[i], alpha, 'ko', markersize=6)
        ax2.plot(sub_right[i], alpha, 'ko', markersize=6)
    ax2.set_xlabel('x', fontsize=12)
    ...
    # Умножение
    for i, alpha in enumerate(alpha_values):
        ax3.plot([mult_left[i], mult_right[i]], [alpha, alpha], 'k-', linewidth=2)
        ax3.plot(mult_left[i], alpha, 'ko', markersize=6)
        ax3.plot(mult_right[i], alpha, 'ko', markersize=6)
    ...
    # Деление
    for i, alpha in enumerate(alpha_values):
        ax4.plot([div_left[i], div_right[i]], [alpha, alpha], 'k-', linewidth=2)
        ax4.plot(div_left[i], alpha, 'ko', markersize=6)
        ax4.plot(div_right[i], alpha, 'ko', markersize=6)
